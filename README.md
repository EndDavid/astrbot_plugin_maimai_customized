...
- `1.2.1` 优化吃分推荐展示与轮换：合并推荐理由和曲目信息图，按用户记录最近 3 次推荐，补充 B50 倾向标签。
...
| `static/score_recommend_history.json` | 吃分推荐按用户记录的最近 3 次推荐谱面 | 不应提交 |
...
| `/吃分推荐` | 按自己的当前 Rating 区间、B35/B15 地板和拟合定数推荐吃分谱面 |
| `/吃分推荐 @用户` | 为被 @ 用户生成吃分推荐 |

吃分推荐会先从水鱼查分器读取当前 B50 和 Rating，再按以下顺序筛选：基础定数范围为 `当前 Rating / 1130` 到 `当前 Rating / 1075`，下限不低于对应 B35/B15 地板换算出的 SSS+ 所需定数，上限不超过 `15.0`；SSS+ 理论单曲 Rating 高于对应 B35/B15 的已有最低正分；谱面未以 SSS+ 出现在 B50 中；最后优先推荐实际定数更低的谱面，实际定数相同时再推荐“拟合定数 - 实际定数”更低的谱面。满足条件的候选超过 3 首时，会按用户避开最近 3 次已经推荐过的谱面，并把推荐理由、B50 倾向标签和曲目信息图合并为一条引用回复。
